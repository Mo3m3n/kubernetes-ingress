...
data:
  ...
  names: |
    ...
    bidule
```
Patterns files are useful in [config-snippets](./README.md#config-snippet):
```
backend-config-snippet: |
  http-request deny if !{ src -f patterns/ips }
```

  :information_source: In order to use pattern files the target name  **should be prefixed with patterns/**
